...
{
    /// <summary>
    /// Summary description for UploadDealerImageHandler
    /// </summary>
    public class UploadDealerImageHandler : IHttpHandler
    {
        ...
        public UploadDealerImageHandler()
        {
        ...
        {
            var statuses = new List<UploadDealerFileStatus>();
            var headers = context.Request.Headers;
        ...
        // Upload partial file
        private void UploadPartialFile(string fileName, HttpContext context, List<UploadDealerFileStatus> statuses)
        {
            ...
            statuses.Add(new UploadDealerFileStatus(new FileInfo(fullName)));
        }

        // Upload entire file
        private void UploadWholeFile(HttpContext context, string uniqueId, List<UploadDealerFileStatus> statuses)
        {
            ...
            for (int i = 0; i < context.Request.Files.Count; i++)
            {
                ...
                statuses.Add(new UploadDealerFileStatus(url, file.ContentLength, fullPath));
                var _agent = NinjectConfig.Get<IDealer>();
                //_agent = new MongoDbRepository.Implementation.Admin
                _agent.UploadProfileImage(uniqueId, url);
            }
        }

        private void WriteJsonIframeSafe(HttpContext context, List<UploadDealerFileStatus> statuses)
        {
        ...
        private void ListCurrentFiles(HttpContext context)
        {
            var files =
                new DirectoryInfo(StorageRoot)
                    ...
                    .Select(f => new UploadDealerFileStatus(f))
                    .ToArray();
